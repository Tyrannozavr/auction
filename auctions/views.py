...
def addwatchlist(request):
    ...
    messages.success(request, "Добавлена в список наблюдений")
...
def deletewatchlist(request):
    ...
    # this message remains untill u reload
    messages.success(request, f"{list_.watch_list.title} удалена из вашего списка наблюдений.")
    list_.delete()
...
def bid(request):
    ...
    if int(bid_amnt) > int(min_req_bid):
        ...
        mybid.save()
        messages.success(request, "Ставка размещена")
        return redirect("index")

    messages.warning(request, f"К сожалению, ставка {bid_amnt}$ меньше. Она должна быть больше, чем {min_req_bid}$.")
...
def win_ner(request):
    ...
    win.save()
    messages.success(request, f"{winners_name} победил со ставкой: {win.bid_win_list.title}.")
    return redirect("index")
